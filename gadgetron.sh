...
for dat in $(find datasets -name "*.dat" -type l); do
    outdir=$(dirname $dat)
    ...
    if [ ! -e $outfile ]; then
        ...
        "timeout --signal=KILL 3h singularity run --cleanenv --writable-tmpfs -B \$TMPDIR:/var/run -B \$TMPDIR:/tmp -B \$TMPDIR -B \$TMPDIR:/var/log/supervisor -B \$PWD:/opt/data -B $project_dir --pwd /opt/data ~/simg/ubuntu_1404_cuda75_latest.sif $project_dir/code/$script $(basename $dat) $port1 $port2 $port3" || true
